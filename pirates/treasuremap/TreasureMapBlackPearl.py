...
from panda3d.core import BitMask32, Camera, CollideMask, CollisionNode, CollisionSphere, Fog, ModelRoot, NodePath, Point3, Vec3, Vec4, headsUp, lookAt
...
class TreasureMapBlackPearl(DistributedTreasureMapInstance.DistributedTreasureMapInstance):
    ...
    def delete(self):
        ...
        if self.newEventSphereNodePath:
            self.newEventSphereNodePath.remove_node_node()
            self.newEventSphereNodePath = None
        ...
        taskMgr.remove_node('fireNPCCannon')
        taskMgr.remove_node('disablePearlInteractions')
    ...
    def stopCutsceneTask(self):
        taskMgr.remove_node('tryToGoToCutscene')
    ...
    def _stopFog(self):
        if hasattr(self, '_fog'):
            taskMgr.remove_node(self._moveFogDownEvent)
    ...
    def endCutscene(self):
        ...
        if self.messageHolder:
            self.messageHolder.remove_node_node()
            self.messageHolder = None
    ...
    def disablePearlInteractions(self, task):
        ...
        localAvatar.guiMgr.mapPage.remove_nodeShip(self.pearl.doId)
    ...
    def getCameraMove1(self):
        ...
        camera.lookAt(firstShip)
        ival = Sequence(Func(self.pauseInjured), Func(aspect2d.hide), Func(base.transitions.letterboxOn), Parallel(LerpPosInterval(camera, 5, pos1, startPos = pos0, blendType = 'easeInOut'), LerpFunc(lerpLookAtAttackShip, 10)), Func(base.transitions.fadeOut), Func(base.transitions.letterboxOff), Wait(1.0), Func(headsUpFirstShip), Func(gotoFps), Func(base.transitions.fadeIn), Func(localAvatar.setH, 120), Func(dummyNode.remove_nodeNode), Func(self.showRulesPanel, 2), Func(base.reviveInput), Func(self.finishInjured), Func(aspect2d.show))
        return ival
