...
class DistributedSimpleShip(DistributedMovingObject, Teamable, DistributedFlagship):
    ...
    placeLocalAvatar = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(placeLocalAvatar)
    ...
    def setLandedGrapples(self, landedGrapples):
        ...
        if landedGrapples:
            ...
            self.pendingGrappledShip = self.cr.relatedObjectMgr.requestObjects([
                shipId], eachCallback = setGrapplesOnModel, timeout = 60)
        else:
            self.removeLandedGrapples()
    ...
    handleChildArrive = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(handleChildArrive)
    
    def battleAvatarArrived(self, av):
        av.wrtReparentTo(self.getModelRoot())

    battleAvatarArrived = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(battleAvatarArrived)
    
    def battleNpcArrived(self, av):
        av.onShipWithLocalAv(self.localAvPresent)

    battleNpcArrived = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(battleNpcArrived)
    
    def playerPirateArrived(self, av):
        pass

    playerPirateArrived = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(playerPirateArrived)
    ...
    def localPirateArrived(self, av):
        ...
        messenger.send('settingLocalShip', [
            True])
    ...
    localPirateArrived = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(localPirateArrived)
    ...
    battleAvatarLeft = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(battleAvatarLeft)
    
    def battleNpcLeft(self, av):
        av.onShipWithLocalAv(False)

    battleNpcLeft = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(battleNpcLeft)
    
    def playerPirateLeft(self, av):
        pass

    playerPirateLeft = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(playerPirateLeft)
    ...
    def localPirateLeft(self, av):
        ...
        if self.isGenerated() and self.cr.activeWorld and self.cr.activeWorld.getType() == PiratesGlobals.INSTANCE_PVP:
            if ShipGlobals.INTERCEPTORL1 <= self.modelClass:
                pass
            self.modelClass <= ShipGlobals.INTERCEPTORL4
            if 1:
    ...
    localPirateLeft = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(localPirateLeft)
    ...
    def _handleEnterAggroSphere(self, collEntry):
        ...
        if localAvatar.ship or localAvatar.ship.getThreatLevel() > EnemyGlobals.SHIP_THREAT_NONE or self.getTeam() in [
    ...
    getRandomBoardingSpot = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(getRandomBoardingSpot)
    
    def getBoardingLocator(self, spotIndex):
        return self.findLocator('**/boarding_spot_%s;+s' % spotIndex)

    getBoardingLocator = report(types = [
        'args'], dConfigParam = [
        'dteleport'])(getBoardingLocator)
    ...
    getRandomBoardingPosH = report(types = [
        'frameCount',
        'deltaStamp'], dConfigParam = 'shipboard')(getRandomBoardingPosH)
