...
MissingAnims = {ShipGlobals.SKEL_WARSHIPL3: ('tiedup', 'rolldown', 'rollup'),
...
class ShipFactory:
    ...
    def __init__(self, phasedLoading = False):
        ...
        if (phasedLoading or self.handlePhase4()):
            self.handlePhase5()
    ...
    def getShip(self, shipClass, style = ShipGlobals.Styles.Undefined, logo = ShipGlobals.Logos.Undefined, hullDesign = None, detailLevel = 2, wantWheel = True, hullMaterial = None, sailMaterial = None, sailPattern = None, prowType = None, invertLogo = False):
        ...
        if (sailPattern or sailMaterial):
            if (hullMaterial or SailReplace.has_key(shipClass)):
                complexCustomization = 1
            if prowType:
                prowType = shipConfig['prow']
            if hullMaterial:
                hullMaterial = style
            if (sailMaterial or SailReplace.has_key(shipClass)):
        ...
        self.notify.debug(('%s %s' % (sailPattern,
         logo)))
        ...
        if ((detailLevel == 0) and lodNode.node().addSwitch(200, 0)):
        ...
        for data in [(0, 'location_mainmast_0'),
         ...
         (4, 'location_foremast*')]:
            ...
            if mastData:
                ...
                if (complexCustomization and model.setTexture(shipTextureSail)):
                    pass
                useLogoTex = logoTex
                if placeLogos:
                    mastNum = data[0]
                ...
                if (data[0] < 3):
                    for side in ['left',
                     'right']:
                        ropeNode = hull.locators.find(('**/location_ropeLadder_%s_%s' % (side,
                         data[0])))
                        if ropeNode:
                ...
                if (sailPatternTex and useLogoTex):
                    for node in model.findAllMatches('**/sails'):
                        ...
                        if (invertLogo and node.setTexture(self.logoLayerInv, logoTex)):
                            pass
                        node.setTexture(self.vertLayer, shipTextureSail)
                        node.setTexture(self.baseLayer, shipTextureSail)

                model.flattenLight()
                if ((detailLevel == 0) and model.find('**/low').copyTo(high)):
                ...
                for (anim, fileName,) in mast.metaAnims.iteritems():
                    ...
                    if (anim not in MissingAnims.get(modelClass, [])):
                        ...
                        if (ac and metaAnims[anim].storeAnim(ac, str(metaAnims[anim].getNumAnims()))):
                            pass
        ...
        if (complexCustomization and hull.geoms[0].setTexture(shipHullTexture)):
        ...
        if (complexCustomization or ship.char.setTexture(shipHullTexture)):
            pass
        return ship
