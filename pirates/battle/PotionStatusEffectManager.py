...
class PotionStatusEffectManager:
        ...
        def startHealEffects():
            if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsMedium:
                ...
                if self._potionSparks:
                    ...
                    self._potionSparks.setEffectColor(Vec4(0.29999999999999999, 1, 1, 0.29999999999999999))
            ...
            if base.options.getSpecialEffectsSetting() >= base.options.SpecialEffectsHigh:
                ...
                if self._potionRays:
                    ...
                    self._potionRays.setEffectColor(Vec4(0.29999999999999999, 1, 1, 1))
    ...
    def _startVomitEffect(self):
        ...
        if self._vomitEffect:
            ...
            self._vomitEffect.setPos(-0.050000000000000003, 0.0, -0.25)
    ...
    def _addVomitEffect(self, attackerId, duration):
        ...
        self._vomitEffectParallel = Sequence(Func(self._ifInBattlePutAwayWeapon), Wait(0.75), Parallel(Func(self.playVomitSfx), self.avatar.actorInterval('barf', playRate = 1.0, blendInT = 0.20000000000000001, blendOutT = 0.20000000000000001), Sequence(Wait(1.0), Func(self._startVomitEffect))))
    ...
    def _addFartEffect(self, attackerId, duration):
        ...
        self._fartEffectParallel = Sequence(Func(self._ifInBattlePutAwayWeapon), Wait(0.75), Parallel(Func(self.playFartSfx), self.avatar.actorInterval('fart', playRate = 1.0, blendInT = 0.20000000000000001, blendOutT = 0.20000000000000001), Sequence(Wait(0.69999999999999996), Func(self._startFartEffect))))
    ...
    def _addBurpEffect(self, attackerId, duration):
        ...
        self._burpEffectParallel = Sequence(Func(self._ifInBattlePutAwayWeapon), Wait(0.75), Parallel(Func(self.playBurpSfx), self.avatar.actorInterval('burp', playRate = 1.0, blendInT = 0.20000000000000001, blendOutT = 0.20000000000000001), Sequence(Wait(1.2), Func(self._startBurpEffect))))
    ...
    def _addHeadFireEffect(self, attackerId, duration):
        ...
        if self._headFireEffect:
            if hasattr(self.avatar, 'headEffects') and self.avatar.headEffects:
                ...
                self._headFireEffect.setPos(0, 0, self.avatar.height * 0.80000000000000004)
            self._headFireEffect.effectScale = 0.29999999999999999 * avatarScale
