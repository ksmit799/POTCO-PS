...
class PowderKeg(Weapon.Weapon):
    ...
    def loadModel(self):
        ...
        self.prop.setPos(-0.29999999999999999, 0.0, -0.80000000000000004)
        self.prop.setHpr(0, 225, -15)
        self.prop.reparentTo(self)

    
    def getDrawIval(self, av, ammoSkillId = 0, blendInT = 0.10000000000000001, blendOutT = 0):
        track = Parallel(av.actorInterval('bigbomb_draw', playRate = 1.5, endFrame = 40, blendInT = blendInT, blendOutT = blendOutT), Func(base.playSfx, self.drawSfx, node = av, cutoff = 60), Sequence(Wait(0.34300000000000003), Func(self.attachTo, av)))
        return track
