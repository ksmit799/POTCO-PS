...
class Grenade(Weapon.Weapon):
    ...
    def changeStance(self, av):
        ...
        if not self.ammoSkillId:
            ...
            self.prop.setPos(-0.050000000000000003, -0.050000000000000003, -0.20000000000000001)
            self.prop.setScale(0.45000000000000001)
            ...
            self.painAnim = 'boxing_hit_head_right'
            self.prop.setPos(-0.14999999999999999, -0.25, -0.5)
    ...
    def getDrawIval(self, av, ammoSkillId = 0, blendInT = 0.10000000000000001, blendOutT = 0):
        if ammoSkillId == InventoryType.GrenadeSiege:
            track = Parallel(av.actorInterval('bigbomb_draw', playRate = 1.5, endFrame = 40, blendInT = blendInT, blendOutT = blendOutT), Func(base.playSfx, self.drawSfx, node = av, cutoff = 60), Sequence(Wait(0.34300000000000003), Func(self.attachTo, av), Func(self.setAmmoSkillId, ammoSkillId), Func(self.changeStance, av)))
        else:
    ...
    def getReturnIval(self, av, ammoSkillId = 0, blendInT = 0, blendOutT = 0.10000000000000001):
        if av.curAttackAnim:
        ...
        if ammoSkillId == InventoryType.GrenadeSiege:
            ...
            track = Parallel(av.actorInterval('bomb_draw', playRate = 1.5, startFrame = 20, endFrame = 1, blendInT = blendInT, blendOutT = blendOutT), Func(base.playSfx, self.returnSfx, node = av, cutoff = 60), Sequence(Wait(0.46800000000000003), Func(self.detachFrom, av), Func(self.setAmmoSkillId, 0), Func(self.changeStance, av)))
        return track
