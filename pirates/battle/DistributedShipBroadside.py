...
class DistributedShipBroadside(DistributedWeapon, DistributedShippart):
        ...
        if side == 0:
            ...
            for index in range(len(self.leftBroadside)):
                if self.leftBroadside[index]:
                    ...
                    playShot = Sequence(Wait(delays[index]), Func(self.setCannonAnim, index, 0, 0), Wait(0.5), Func(self._DistributedShipBroadside__requestAttack, index, side, targetPos, flightTime, fireSecondary = fireSecondary), Wait(3.0), Func(self.setCannonAnim, index, 0, 1))
                    playShot.start(ts)
            ...
            for index in range(len(self.rightBroadside)):
                if self.rightBroadside[index]:
                    ...
                    playShot = Sequence(Wait(delays[index]), Func(self.setCannonAnim, index, 1, 0), Wait(0.5), Func(self._DistributedShipBroadside__requestAttack, index, side, targetPos, flightTime, fireSecondary = fireSecondary), Wait(3.0), Func(self.setCannonAnim, index, 1, 1))
                    playShot.start(ts)
    ...
    def _DistributedShipBroadside__requestAttack(self, index, side, targetPos, flightTime, fireSecondary = False):
        if side == 0:
            if self.leftBroadside and self.leftBroadsideConfig:
                cannonList = self.leftBroadside
