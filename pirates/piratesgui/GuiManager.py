...
class GuiManager(FSM.FSM):
    ...
    def openShipUpgrades(self, shipId, callback = None):
        ...
        self.shipUpgradeInterface.setPos(-0.29999999999999999, 0.0, -0.90000000000000002)
        self.shipUpgradeInterface.show()
    ...
    def doInstructionMessageQueue(self, task = None):
        ...
        if len(self.instructionMessageQueue) > 0:
            ...
            if not (base.firstMateVoiceOn) and messageCategory in [
                MessageGlobals.MSG_CAT_THREAT_LEVEL,
                MessageGlobals.MSG_CAT_NO_PORT,
                MessageGlobals.MSG_CAT_TELL_PORT,
                MessageGlobals.MSG_CAT_ANNOUNCE_ATTACK,
                MessageGlobals.MSG_CAT_SUNK_SHIP]:
                pass
            1
    ...
    def toggleSocialPanel(self, args = None):
        if self.ignoreAllKeys and not (self.seaChestAllowed) or not self.getSocialPanelAllowed():
            return None
    ...
    def toggleMainMenu(self, args = None):
        ...
        elif self.av.getGameState() not in ('Cutscene', 'Dialog', 'NPCInteract', 'DinghyInteract', 'MakeAPirate'):
            self.chatPanel.show()
            self.av.chatMgr.start()
            self.av.startChat()
            self.setSeaChestAllowed(True)
    ...
    def getSocialPanelAllowed(self):
        if self.getTutorialStatus() >= PiratesGlobals.TUT_MET_JOLLY_ROGER:
            pass
        return self.socialPanelAllowed
