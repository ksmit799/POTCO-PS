    ...
    def getBestTonic(self, allowNone = 0):
        ...
        if inv and inv.isReady():
            ...
            for (tonicId, count) in sorted(tonics.iteritems()):
                ...
                WeaponGlobals.getAttackSelfHP(skillId) > idealAmount
...
class CombatTray(GuiTray):
    ...
    def processInput(self, button):
        ...
        if not inventory:
            ...
            not (WeaponGlobals.getAttackClass(skillId) == WeaponGlobals.AC_COMBAT)
            if self.weaponMode == WeaponGlobals.GRENADE:
                self.trySkill(skillId, self.ammoSkillId, charge = self.chargeTime)
                return None
    ...
    def skillDisabled(self, skillId):
        if (skillId == InventoryType.SailPowerRecharge and skillId == InventoryType.SailRammingSpeed and skillId == InventoryType.SailTakeCover or skillId == InventoryType.SailOpenFire) and localAvatar.ship:
            pass
