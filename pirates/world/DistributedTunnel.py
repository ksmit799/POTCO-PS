...
class DistributedTunnel(DistributedNode, StagedObject):
    ...
    def setLoadedArea(self, area):
        if self._DistributedTunnel__loadedArea and self._DistributedTunnel__loadedArea.isGenerated():
            self._DistributedTunnel__loadedArea.builder.removeLargeObj(self.visNodes[self._DistributedTunnel__loadedArea.getUniqueId()], self.uniqueId)
        
        if area:
            self._DistributedTunnel__loadedArea = area
            self._DistributedTunnel__loadedArea.builder.addLargeObj(self.visNodes[self._DistributedTunnel__loadedArea.getUniqueId()], self.uniqueId)
        

    
    def getLoadedArea(self):
        return self._DistributedTunnel__loadedArea
    ...
    calcOneAmbientName = report(types = [
        'deltaStamp',
        'args'], dConfigParam = 'connector')(calcOneAmbientName)
    ...
    setupConnectorNodes = report(types = [
        'args'], dConfigParam = 'connector')(setupConnectorNodes)
    ...
    def setupCollisions(self):
        ...
        for (x, floorName) in enumerate(floorNames):
            ...
            self.accept('enterFloor' + floor.getName(), self._DistributedTunnel__handleOnFloor, extraArgs = [
                x])
    ...
    def __handleOnFloor(self, areaIndex, collEntry):
        if not self._inTransition:
            ...
            if collEntry:
                self.acceptOnce('EnterTunnelFinished', self.transitionToArea, extraArgs = [
                    self.floorIndex])
    ...
    _DistributedTunnel__handleOnFloor = report(types = [
        'deltaStamp',
        'args'], dConfigParam = 'connector')(__handleOnFloor)
    ...
    def transitionToArea(self, areaIndex):
        ...
        def continueTransition():
            ...
            if currParentObj:
                ...
                self.cr.activeWorld.goOffStage([
                    self.getLoadedArea()])
    ...
    transitionToArea = report(types = [
        'deltaStamp',
        'args'], dConfigParam = 'connector')(transitionToArea)
    ...
    handleOffStage = report(types = [
        'frameCount'], dConfigParam = 'connector')(handleOffStage)
    ...
    def d_requestArea(self, linkIndex):
        self.sendUpdate('requestArea', [
            self._links[linkIndex][self.L_LINK_NODE]])

    d_requestArea = report(types = [
        'frameCount',
        'args'], dConfigParam = 'connector')(d_requestArea)
    ...
    def quickLoadOtherSide(self):
        ...
        if self.floorIndex != -1:
            self._inTransition = False
            self._DistributedTunnel__handleOnFloor(self.floorIndex, None)
