...
class DistributedIsland(DistributedGameArea, DistributedCartesianGrid, ZoneLOD, Teamable, MappableGrid):
    ...
    def loadZoneLevel(self, level):
        if level == 0:
            ...
            self.cr.addTaggedInterest(self.doId, PiratesGlobals.IslandShipDeployerZone, self.cr.ITAG_GAME, 'ShipDeployer', otherTags = [
                'ShipDeployer'])
            messenger.send('toggleIslandNametag', [
                1])
    ...
    placeOnMap = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'map')(placeOnMap)
    ...
    removeFromMap = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'map')(removeFromMap)
    ...
    setOceanVisEnabled = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'connector')(setOceanVisEnabled)
    ...
    def setZoneLevel(self, *args, **kw):
        ZoneLOD.setZoneLevel(self, *args, **args)
