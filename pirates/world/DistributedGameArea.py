...
class DistributedGameArea(DistributedNode, MappableArea, StagedObject):
    ...
    def __repr__(self):
        if not self.getName():
            pass
        return '%s (%s)' % (DistributedNode.__repr__(self), self.uniqueId)
                ...
                if connectorId not in self.connectorInterests or not self.cr.doId2do.has_key(connectorId):
                    ...
                    def connectorParentHere(connectorId, parentId, zoneId):
                        ...
                        self.acceptOnce(connectorEvent, self.reparentConnector, extraArgs = [
                            connectorId])
                        self.cr.addTaggedInterest(parentId, zoneId, self.cr.ITAG_GAME, 'Connectors-%s' % self.doId, event = connectorEvent, otherTags = [
                            'Connectors-%s' % self.doId])

                    connectorEvent = 'connector-%s' % connectorId
                    self.acceptOnce(connectorEvent, connectorParentHere, extraArgs = [
                        connectorId,
                        parentId,
                        zoneId])
                    self.cr.addTaggedInterest(linkParentId, linkParentZoneId, self.cr.ITAG_GAME, 'ConnectorsParent-%s' % self.doId, event = connectorEvent, otherTags = [
