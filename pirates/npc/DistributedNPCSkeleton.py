...
class DistributedNPCSkeleton(DistributedBattleNPC.DistributedBattleNPC, Skeleton.Skeleton):
    ...
    def play(self, *args, **kwArgs):
        Skeleton.Skeleton.play(self, *args, **args)

    
    def loop(self, *args, **kwArgs):
        Skeleton.Skeleton.loop(self, *args, **args)

    
    def pose(self, *args, **kwArgs):
        Skeleton.Skeleton.pose(self, *args, **args)

    
    def pingpong(self, *args, **kwArgs):
        Skeleton.Skeleton.pingpong(self, *args, **args)

    
    def stop(self, *args, **kwArgs):
        Skeleton.Skeleton.stop(self, *args, **args)
    ...
    def getTurnAnim(self, noticeDif):
        if self.avatarType.getTrack() in [
            0]:
            return 'crazy_idle'
        else:
