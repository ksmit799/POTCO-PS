...
class InventoryUICardContainer(InventoryUIStackContainer.InventoryUIStackContainer):
    ...
    def setTitleInfo(self):
        ...
        self.titleImageOpen.setScale(0.40000000000000002)
        self.titleImageClosed.setScale(0.40000000000000002)
    ...
    def resizeItem(self, item):
        ...
        item.setPos(0, 0, 0)
        item['text_pos'] = (0.050000000000000003, 0.042999999999999997)
        item['geom_pos'] = (0.050000000000000003, 0, 0.050000000000000003)
        if item.getAmount() == 0:
            scale = 2.0 * self.cellSizeX * item.imageScale
            item['image_scale'] = [
                scale,
                scale,
                scale * 0.95999999999999996 / 0.70999999999999996]
            ...
            item.setPos(0.025000000000000001, 0, 0)
            item['text_pos'] = (0.080000000000000002, 0.042999999999999997)
            item['geom_pos'] = (0.080000000000000002, 0, 0.050000000000000003)
    ...
    def findGridPos(self, x, z, wantGap = 0):
        offX = 0.0
        offZ = -0.050000000000000003
        return Point3(offX + float(x) * self.cellSizeX * 0.20999999999999999, 0, offZ + float(z) * self.cellSizeZ * 0.035000000000000003)

    
    def getCell(self, bottomCard = 0):
        if bottomCard:
            ammoCell = self.makeCell(frameScale = (0.11, 0.089999999999999997), offset = (0.0, 0.0))
        else:
            ammoCell = self.makeCell(frameScale = (0.11, 0.035000000000000003), offset = (0.0, 0.050000000000000003))
        return ammoCell
