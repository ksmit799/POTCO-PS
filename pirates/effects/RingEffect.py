...
class RingEffect(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        self.track = Sequence(Func(self.p0.setBirthRate, 0.14999999999999999), Func(self.p0.clearToInitial), Func(self.f.start, self, self), Parallel(fadeBlast, scaleBlast), Func(self.p0.setBirthRate, 100.0), Wait(3.0), Func(self.cleanUpEffect))

    
    def setEffectScale(self, scale):
        self.effectScale = scale
        self.p0.renderer.setInitialXScale(1.3999999999999999 * self.cardScale * scale)
        self.p0.renderer.setFinalXScale(1.2 * self.cardScale * scale)
        self.p0.renderer.setInitialYScale(1.3999999999999999 * self.cardScale * scale)
