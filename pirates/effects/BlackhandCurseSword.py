...
class BlackhandCurseSword(PooledEffect, EffectController):
    ...
    def __init__(self):
        ...
        self.p0.renderer.setInitialYScale(0.0011999999999999999 * self.cardScale)
        self.p0.renderer.setFinalXScale(0.00029999999999999997 * self.cardScale)
        self.p0.renderer.setFinalYScale(0.00029999999999999997 * self.cardScale)
    ...
    def createTrack(self):
        ...
        self.startEffect = Sequence(Func(self.p0.setBirthRate, 0.014999999999999999), Func(self.p0.clearToInitial), Func(self.effectActor.loop, 'spin'), Func(self.f.start, self, self.particleDummy), Func(self.f.reparentTo, self), Func(scalePulse.loop), Func(self.p1.setBirthRate, 0.01), Func(self.p1.clearToInitial), Func(self.f2.start, self.joint, self))
        ...
        self.track = Sequence(Func(scalePulse.loop), Wait(2.5), self.startEffect, Wait(0.69999999999999996), Func(self.setOffsetForce), Wait(5.75), Func(self.setOffsetForce), Wait(2.0), Func(self.p0.emitter.setOffsetForce, Vec3(0.0, 0.0, 0.5)), Wait(1.5), self.endEffect)
