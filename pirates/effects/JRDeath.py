...
class JRDeath(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        fadeInRays = LerpColorScaleInterval(self.rays, 0.20000000000000001, VBase4(0.80000000000000004, 1, 0.10000000000000001, 1), startColorScale = VBase4(0.80000000000000004, 1, 0.10000000000000001, 0))
        fadeOutRays = LerpColorScaleInterval(self.rays, 0.75, VBase4(0.80000000000000004, 1, 0.10000000000000001, 0), startColorScale = VBase4(0.80000000000000004, 1, 0.10000000000000001, 1))
        scaleUpRays = LerpScaleInterval(self.rays, 0.20000000000000001, VBase3(1, 1, 1), startScale = VBase3(1, 1, 0), blendType = 'easeOut')
        scaleDownRays = LerpScaleInterval(self.rays, 0.75, VBase3(1, 1, 0), startScale = VBase3(1, 1, 1), blendType = 'easeIn')
        scaleJollyIval = LerpScaleInterval(self.jolly, 3.5, Vec3(1.5, 1.0, 4.0), startScale = Vec3(0.40000000000000002, 1.0, 0.75))
        textureStage = self.jolly.findAllTextureStages()[0]
        uvScrollA = LerpFunctionInterval(self.setNewUVs, 1.0, toData = -2.6000000000000001, fromData = 0.0, extraArgs = [
            self.jolly,
            textureStage])
        uvScrollB = LerpFunctionInterval(self.setNewUVs, 0.75, toData = -6.5, fromData = -2.6000000000000001, extraArgs = [
            self.jolly,
            textureStage])
        self.track = Sequence(Func(self.anim.loop), Parallel(fadeInRays, scaleUpRays, scaleJollyIval, Sequence(uvScrollA, uvScrollB), Sequence(Wait(0.20000000000000001), Parallel(fadeOutRays, scaleDownRays))), Func(self.anim.finish), Func(self.cleanUpEffect))
